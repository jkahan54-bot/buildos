...
export async function POST(req: NextRequest) {
  ...
  if (!canApprove) return NextResponse.json({ error: "Only PM/Admin can approve items" }, { status: 403 });
  ...
  if (action === "approve" && itemId) {
    ...
    await admin.from("punch_list_items").update(update).eq("id", itemId);
    return NextResponse.json({ success: true });
  }

  if (action === "reject" && itemId) {
    // Reject — delete the item entirely
    await admin.from("punch_list_items").delete().eq("id", itemId);
    return NextResponse.json({ success: true });
  }
