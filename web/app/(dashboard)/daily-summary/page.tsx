        ...
        <div className="bg-white rounded-xl border border-blue-200 shadow-sm overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
              {(report.whatsapp_count ?? 0) > 0 && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-green-50 text-green-700">📱 {report.whatsapp_count}</span>
              )}
              {(report.email_count ?? 0) > 0 && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">📧 {report.email_count}</span>
              )}
              {(report.completed_count ?? 0) > 0 && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">✅ {report.completed_count} done</span>
              )}
              {(report.incident_count ?? 0) > 0 && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-700">🚨 {report.incident_count}</span>
              )}
          ...
          {reportOpen && (
            <div className="border-t border-blue-100 px-5 py-4 space-y-4">
                ...
                {report.projects.map((p, i) => (
                  <div key={i} className="bg-gray-50 rounded-lg px-4 py-3">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {(p.whatsapp_count ?? 0) > 0 && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-green-50 text-green-700">📱 {p.whatsapp_count} WhatsApp</span>
                      )}
                      {(p.email_count ?? 0) > 0 && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-700">📧 {p.email_count} email</span>
                      )}
                    </div>
              ...
              <div className="flex items-center gap-4 pt-1 flex-wrap">
                {(report.blocker_count ?? 0) > 0 && (
                  <span className="text-[11px] text-orange-600 font-medium">⏳ {report.blocker_count} open blocker{report.blocker_count !== 1 ? "s" : ""}</span>
                )}
                {(report.completed_count ?? 0) > 0 && (
                  <span className="text-[11px] text-green-600 font-medium">✅ {report.completed_count} completed today</span>
                )}
                <span className="text-[10px] text-gray-400 ml-auto">Nightly scan · {report.report_date}</span>
              </div>
            </div>
          )}
        </div>
